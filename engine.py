...
def convert_decimal_to_american(decimal_val):
    try:
        ...
        if num <= 1.0:
            return "+100"
        if num >= 2.0:
            val = round((num - 1.0) * 100)
            return f"+{val}"
        else:
            val = round(-100 / (num - 1.0))
            return str(val)
    except Exception:
        return "+100"
...
def parse_multi_market_odds(bookmaker_data):
    ...
    # Tier 1: Look for Full-Time Moneyline / Result keywords
    for m in markets:
        k = str(m.get("key", "")).lower()
        if any(x in k for x in ["h2h", "winner", "result", "three_way"]):
            for outcome in m.get("outcomes", []):
                n = outcome.get("name")
                p = outcome.get("price")
                if is_any_valid_market_selection(p) and "." in str(p):
                ...
                elif any(x in str(n).lower() for x in ["draw", "tie", "x"]) or outcome.get("side") == "draw": odds_map["draw"] = p
            odds_map["market_used"] = f"FT_ML ({k})"
            if odds_map["home"] is not None: return odds_map

    # Tier 2: Look for 1st Half / Halftime Results
    for m in markets:
        k = str(m.get("key", "")).lower()
        if any(x in k for x in ["1h", "half", "halftime"]):
            for outcome in m.get("outcomes", []):
                n = outcome.get("name")
                p = outcome.get("price")
                if is_any_valid_market_selection(p) and "." in str(p):
            ...
            odds_map["market_used"] = f"1H_ML ({k})"
            if odds_map["home"] is not None: return odds_map

    # Tier 3: Look for Totals, Goals, Spreads, Handicaps, Asian lines
    for m in markets:
        k = str(m.get("key", "")).lower()
        if any(x in k for x in ["goal", "goals", "totals", "handicap", "spreads", "asian", "over", "under"]):
            for outcome in m.get("outcomes", []):
                n = outcome.get("name")
                if any(x in str(n).lower() for x in ["over", "home", "away", "plus", "minus"]):
                    p = outcome.get("price")
                    if is_any_valid_market_selection(p) and "." in str(p):
                        p = convert_decimal_to_american(p)
                    odds_map["home"] = p
                    odds_map["away"] = p
                    odds_map["draw"] = "+200"
                    odds_map["market_used"] = f"ALT_MKT ({k})"
                    return odds_map
...
def get_league_standings_and_audit(league_id, home_team, away_team):
    ...
    for season in seasons_to_check:
        try:
            if not league_id or str(league_id) == "9999":
                break
                    ...
                    if standings_lists and isinstance(standings_lists, list):
                        ...
                        if h_found or a_found:
                            ...
                            if season == current_year and games_played <= 5:
                                continue # Trigger 3-year lookback fallback logic safely
...
def execute_global_pitch_sweeps():
    ...
    # -----------------------------------------------------------------
    # PIPELINE 2: All-Leagues All-Tiers Dual State Footprint Sweep
    # -----------------------------------------------------------------
    try:
        # Step 1: Poll current active competition endpoints to extract keys cleanly (Consuming 1 credit total)
        leagues_index_url = "https://api.the-odds-api.com/v4/sports"
        leagues_res = requests.get(leagues_index_url, params={"apiKey": LIVE_DATA_API_KEY}, timeout=5)
        active_soccer_keys = []
        if leagues_res.status_code == 200:
            for item in leagues_res.json():
                if item.get("group") == "Soccer" and item.get("active", True):
                    active_soccer_keys.append(item.get("key"))
        
        # Guard fallback if index is empty
        if not active_soccer_keys:
            active_soccer_keys = ["soccer_usa_major_league_soccer", "soccer_uefa_nations_league", "soccer_epl"]

        # Step 2: Loop dynamically through each valid state league mapping key
        for sport_key in active_soccer_keys:
            odds_url = f"https://api.the-odds-api.com/v4/sports/{sport_key}/odds"
            params = {
                "apiKey": LIVE_DATA_API_KEY, 
                "regions": "us,us2", # Mandatory integration to scrape modern state providers cleanly
            ...
            for fixture in res.json():
                ...
                # Localized Timezone Offset Alignment Formatter (Normalizing server UTC to CST calendar boundaries)
                ...
                league_title = fixture.get("sport_title", "All Tiers Loop")
                
                if commence_dt <= current_time_utc: continue 
                total_matches_found += 1
                
                # Sequentially checks specific Illinois & Florida program lists before processing
                target_bookmaker = None
                bookmakers_list = fixture.get("bookmakers", [])
                preferred_order = ["bet365", "draftkings", "fanduel", "thescore", "circa", "hardrock", "williamhill_us", "pointsbetus", "sugarhouse"]
                
                for b_key in preferred_order:
                ...
                if time_delta_to_kickoff > datetime.timedelta(hours=48):
                    ...
                    try:
                        ...
                        full_alert = (
                            ...
                            f"**Match Context:** {home} vs {away} ({league_title}) \u2014 Upcoming Strategy Slate\n"
                            f"\U0001F4C8 **Verified Market Consensus Lines (American Odds):**\n"
                            f"* **Full-Time Moneyline Matrix:** Home: {home_odds_val} | Draw: {draw_odds_val} | Away: {away_odds_val}\n"
                            f"* **Selected Aggregation Anchor:** Verified Line via {odds_payload.get('market_used')} market node.\n\n"
                            f"* **Target Edge Selection Metric ({home} ML):** Implied Probability: {implied_p:.1%} vs Historical Lineage Target Baseline.\n"
                            f"\U0001F3CE **CORVETTE FUND BLUEPRINT \u2014 MARKET ANALYSIS SYSTEM**\n\n"
                            f"**Match Context:** {home} vs {away} ({league_title})\n"
                            f"{system_5_details}\n"
                            f"5. **Live Threat Matrix Edge:** System 4 Lineage fallback loops and xG models match entry variance thresholds. Operational targets initialized safely."
                        )
                        send_discord_payload(full_alert)
                    except Exception as inner_err:
                        print(f"[-] Evaluation display error: {inner_err}")
    except Exception as e:
        print(f"[-] Pre-Match dynamic check fault: {e}")
