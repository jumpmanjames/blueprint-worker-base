...
# PRIORITY SPORTSBOOK KEYS (MAPPED BY EXACT FEEDS)
PRIORITY_BOOKMAKERS = [
    "bet365",        # Bet365
...
def parse_multi_market_odds(bookmaker_data):
    ...
    # Adaptive structure normalization for lists vs dict formats
    ...
    # OMNI-MARKET INTERCEPTOR: Broad string keyword containment
    # Tier 1: Look for Full-Time Moneyline / Win / 1X2 / 3-Way Match Result
    for m in markets:
        key_str = str(m.get("key", "")).lower()
        if "h2h" in key_str or "win" in key_str or "result" in key_str:
            for outcome in m.get("outcomes", []):
                ...
                elif "draw" in n.lower() or "tie" in n.lower() or outcome.get("side") == "draw": odds_map["draw"] = p
            odds_map["market_used"] = "FT_Moneyline"
            if odds_map["home"] is not None: return odds_map

    # Tier 2: Look for 1st Half Moneylines / Halftime Result / Half Betting
    for m in markets:
        key_str = str(m.get("key", "")).lower()
        if "1h" in key_str or "half" in key_str or "halftime" in key_str:
            if "goal" not in key_str and "total" not in key_str and "handicap" not in key_str:
                for outcome in m.get("outcomes", []):
                    n = outcome.get("name", "")
                    p = outcome.get("price")
                    if isinstance(p, float):
                ...
                odds_map["market_used"] = "1H_Result"
                if odds_map["home"] is not None: return odds_map

    # Tier 3: Look for Totals / Goal Lines / Spreads / Handicaps / Asian Lines
    for m in markets:
        key_str = str(m.get("key", "")).lower()
        if any(x in key_str for x in ["total", "goal", "handicap", "spread", "asian"]):
            for outcome in m.get("outcomes", []):
                n = outcome.get("name", "")
                if "over" in n.lower() or "home" in n.lower() or "+" in n.lower():
                    p = outcome.get("price")
                    if isinstance(p, float):
                        p = convert_decimal_to_american(p)
                    odds_map["home"] = p
                    odds_map["away"] = p if not isinstance(p, int) else p + 50
                    odds_map["draw"] = "+220"
                    odds_map["market_used"] = f"Goal_Handicap_Total ({key_str})"
                    return odds_map
                        ...
                        if odds_res.status_code == 200:
                            for entry in odds_res.json().get("response", []):
                                for mkt in entry.get("odds", []):
                                    if "winner" in str(mkt.get("name", "")).lower() or "result" in str(mkt.get("name", "")).lower():
                                        for values in mkt.get("values", []):
                                            val_str = str(values.get("value", "")).lower()
                                            if "home" in val_str: live_home_odds = values.get("odd")
                                            elif "away" in val_str: live_away_odds = values.get("odd")
                                            elif "draw" in val_str or "tie" in val_str: live_draw_odds = values.get("odd")
    ...
    try:
        ...
        if live_res.status_code == 200:
            ...
            for fx in fixtures_list:
                ...
                # UNIVERSAL IN-PLAY CAPTURE RULE: Ingest ALL soccer lines
                if matched_catalog_item or fx_league_id:
                    total_matches_found += 1
                    league_title = matched_catalog_item["title"] if matched_catalog_item else f"Global League ({fx_league_id})"
                    league_api_id = matched_catalog_item["api_id"] if matched_catalog_item else fx_league_id
                    ...
                    if current_minute >= 45 and current_score == "0-0":
                        ...
                        interval_alert = f"""🏎 **CORVETTE FUND BLUEPRINT — LIVE STRATEGY SIGNAL**
...
* **Why the data holds the edge:** Game clock verified at {live_data.get('clock')} mark sitting at balanced scoreline ({current_score}). Live attack velocity registers {live_data.get('dang_attacks_home')} Dangerous Attacks."""
    ...
    # -----------------------------------------------------------------
    # PIPELINE 2: Broad Pre-Match Horizon Ingestion via The Odds API Global Stream
    # -----------------------------------------------------------------
    url = "https://api.the-odds-api.com/v4/sports/soccer/odds"
    params = {
        "apiKey": LIVE_DATA_API_KEY, 
        "regions": "us", 
        "markets": "h2h,totals,h2h_1h,spreads", 
        "oddsFormat": "american",
    ...
    if isinstance(match_data, list):
        for fixture in match_data:
            ...
            # UNIVERSAL SPORT INGESTION KEY: Capture all soccer competitions
            ...
            matched_catalog_item = None
            for sport_item in MASTER_BOOKIE_CATALOG:
                if sport_item["key"] == sport_key:
                    matched_catalog_item = sport_item
                    break
            
            total_matches_found += 1
            league_title = matched_catalog_item["title"] if matched_catalog_item else f"Global Feed ({sport_key})"
            league_api_id = matched_catalog_item["api_id"] if matched_catalog_item else 9999
                
            # PRIORITIZED SPORTSBOOK MATRIX SELECTION + FALLBACK
            target_bookmaker = None
            bookmakers_list = fixture.get("bookmakers", [])
            
            # 1st Pass: Sequential scanning for user preference layout strings
            for preferred_book in PRIORITY_BOOKMAKERS:
                for bm in bookmakers_list:
                    if str(bm.get("key", "")).lower() == preferred_book:
                        target_bookmaker = bm
                        break
                if target_bookmaker:
                    break
                    
            # 2nd Pass: Universal fallback if top-tiers are locked
            ...
            if time_delta_to_kickoff > datetime.timedelta(hours=48):
                ...
                juice_alert = f"""🏎 **CORVETTE FUND BLUEPRINT — SYSTEM 2 JUICE OVERRIDE**

**Match Context:** {home} vs {away} ({league_title})
...
🎯 **Operational Mandate:** Bypass direct standard line. Execute Time-Bracket strategy entry: **Goal Before 30:00** or **Favorite to Lead Before 30:00**."""
                ...
                try:
                    ...
                    full_alert = f"""🏎 **CORVETTE FUND BLUEPRINT — MARKET ANALYSIS SYSTEM**

**Match Context:** {home} vs {away} ({league_title}) — Upcoming: {commence_dt.strftime('%b %d at %I:%M %p')} Central
...
🏎 **CORVETTE FUND BLUEPRINT — MARKET ANALYSIS SYSTEM**
**Match Context:** {home} vs {away} ({league_title})
{system_5_details}
...
"
    ...
    if all_discovered_favorites:
        ...
        board_msg = f"🏎 **CORVETTE FUND BLUEPRINT — TOP 20 DAILY FAVORITES BOARD**
...
if __name__ == "__main__":
    ...
    while True:
        ...
        test_payload = f"""🏎 **CORVETTE FUND ENGINE — STATUS VERIFIED**
