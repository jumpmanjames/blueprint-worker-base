...
MASTER_BOOKIE_CATALOG = [
    ...
    {"key": "soccer_chile_campeonato", "title": "Chile Liga de Primera", "api_id": 265, "aliases": ["chile liga de primera", "primera division", "campeonato nacional"]},
    ...
    {"key": "soccer_czech_republic_first_league", "title": "Czechia First League", "api_id": 345, "aliases": ["czechia first league", "czech first league"]},
    ...
    {"key": "soccer_greece_super_league", "title": "Greece Super League 1", "api_id": 197, "aliases": ["greece super league 1", "super league"]},
    ...
    {"key": "soccer_poland_ekstraklasa", "title": "Poland Ekstraklasa", "api_id": 106, "aliases": ["poland ekstraklasa", "ekstraklasa"]},
    {"key": "soccer_romania_liga1", "title": "Romania Liga I", "api_id": 283, "aliases": ["romania liga i", "liga i", "romanian liga 1"]},
    {"key": "soccer_serbia_super_liga", "title": "Serbia Super Liga", "api_id": 286, "aliases": ["serbia super liga", "super liga"]},
    ...
    {"key": "soccer_sweden_allsvenskan", "title": "Sweden Allsvenskan", "api_id": 113, "aliases": ["sweden allsvenskan", "allsvenskan"]},
    ...
    try:
        ...
        if res.status_code == 200:
            for fx in res.json().get("response", []):
                ...
                # Loose telemetry string matching fix
                h_match = home_team.lower()[:4] in h.lower() or h.lower()[:4] in home_team.lower()
                a_match = away_team.lower()[:4] in a_team.lower() or a_team.lower()[:4] in away_team.lower()
                ...
                if h_match or a_match:
                    ...
                    hs, as_ = {}, {}
                    for sg in sl:
                        ts = "home" if sg.get("team", {}).get("name", "").lower() == h.lower() else "away"
                    ...
                    if standings_lists and isinstance(standings_lists, list) and len(standings_lists) > 0:
                        ...
                        if h_found or a_found:
                            ...
                            if season == current_year and games_played <= 5:
                                print(f"[!] Current season data too shallow ({games_played} games). Tripping fallback.")
                                continue
            ...
            if live_res.status_code == 200:
                ...
                if fixtures_list:
                    ...
                    for fx in fixtures_list:
                        ...
                        # Match logic parameters fixed to prevent telemetry dropouts
                        ...
                        if live_data.get("active"):
                            ...
                            # Standardized parsing loops for clean extraction
                            if is_any_valid_market_selection(l_home_odds):
                                raw_odds = int(str(l_home_odds).replace("+", ""))
                                all_discovered_favorites.append({"team": h_name, "odds": raw_odds, "match": f"{h_name} vs {a_name}", "league": league_title})
                            if is_any_valid_market_selection(l_away_odds):
                                raw_odds = int(str(l_away_odds).replace("+", ""))
                                all_discovered_favorites.append({"team": a_name, "odds": raw_odds, "match": f"{h_name} vs {a_name}", "league": league_title})
