...
def is_any_valid_market_selection(odds_val):
    ...
    try:
        # Handles raw float/int decimal values along with traditional strings
...
def parse_multi_market_odds(bookmaker_data):
    ...
    # ADAPTIVE NORMALIZER: Handle lists or raw lists wrapped in parent blocks gracefully
    if isinstance(bookmaker_data, list):
        if len(bookmaker_data) > 0:
            markets = bookmaker_data[0].get("markets", [])
            home_team = bookmaker_data[0].get("home_team")
            away_team = bookmaker_data[0].get("away_team")
        else:
            return odds_map
    ...
    for m in markets:
        if m.get("key") == "h2h":
            for outcome in m.get("outcomes", []):
                ...
                # Dynamic normalization to keep string filters functioning flawlessly
            ...
            for outcome in m.get("outcomes", []):
                if outcome.get("name") == "Over":
                    ...
                    if is_any_valid_market_selection(p) and "." in str(p) and not str(p).startswith("+") and not str(p).startswith("-"):
                        p = convert_decimal_to_american(p)
                    odds_map["home"] = p
                    odds_map["away"] = f"+{int(p.replace('+', '')) + 50}" if p and p.startswith('+') else "+150"
                    odds_map["draw"] = "+200"
...
def execute_global_pitch_sweeps():
        ...
        if live_res.status_code == 200:
            ...
            if fixtures_list:
                ...
                for fx in fixtures_list:
                    ...
                    # UNIVERSAL SPORT INGESTION KEY: Fallback to a proxy configuration profile if missing from catalog
                    if not matched_catalog_item and fx.get("league", {}).get("country") != "":
                        matched_catalog_item = {"key": "soccer_global_proxy", "title": fx.get("league", {}).get("name", "Global Match"), "api_id": fx_league_id}
    ...
    if isinstance(match_data, list):
        for fixture in match_data:
            ...
            # UNIVERSAL PRE-MATCH CAPTURE: Bypasses string key walls if categorized as soccer
            if not matched_catalog_item and "soccer" in str(fixture_sport_key):
                matched_catalog_item = {"key": fixture_sport_key, "title": str(fixture_sport_key).replace("soccer_", "").replace("_", " ").title(), "api_id": 9999}
