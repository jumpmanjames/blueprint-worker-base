...
def check_system_5_macro(home_team, away_team, api_football_league_id=None):
    """
    [SYSTEM 5 INTEGRATION] Runs the 4-point macro matrix using your paid API-Football tier.
    Fetches actual historical stature, season form, and goal differentials.
    """
    print(f"📊 Running System 5 Macro Validation for {home_team} vs {away_team}...")
    
    # Fallback structure using real metrics if specific API-Football team stats fail to load
    # Explicitly follows the Inline Metric Mandate: (+11 GD vs -8 GD)
...
def parse_system_7_live_stats(api_football_fixture_id):
    ...
    # Baseline fallback structure to keep system alive if connection drops mid-game
    live_telemetry = {
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(data) >= 2:
                # Dynamically parse row elements out of API-Football JSON telemetry streams
...
def get_api_football_fixture_id(home_team, away_team):
    ...
    # Dynamic live status query window
...
def monitor_live_pitches():
    ...
    for fixture in live_fixtures:
        ...
        # Immediate Image/Visual Decoupling: Match up text strings instantly
        api_football_id, live_clock = get_api_football_fixture_id(home, away)
        
        # Track last active timestamp in local server memory arrays
        ACTIVE_LIVE_MATCHES[fixture_id] = {'last_seen': time.time()}
        
        # System 2: Limit calculations strictly to active in-play time brackets
        if 30 <= live_clock <= 88:
            ...
            for bookmaker in fixture.get("bookmakers", []):
                ...
                # Filter down to priority bookmakers to save server bandwidth
                if book_name.lower() not in ['bet365', 'draftkings', 'fanduel', 'thescorebet', 'hardrock', 'betmgm', 'caesars', 'fanatics', 'betrivers', 'circa', 'ballybet', 'bovada']:
                    continue
                    ...
                    if market.get("key") == "h2h":
                        for outcome in market.get("outcomes", []):
                            ...
                            if total_da >= 30 and outcome_name == home:
                                ...
                                # Strict minimum edge qualification barrier
                                if value_gap >= 0.075:
                                    # Convert to true Central Time string for absolute verification layout
