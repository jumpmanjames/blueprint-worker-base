...
def evaluate_live_inplay_telemetry(fixture_id, match_details, h2h_odds_matrix):
    ...
    for target_book in BOOKMAKER_PRIORITY:
        ...
        if book_match:
            ...
            if markets and len(markets) > 0 and 'outcomes' in markets[0]:
...
def execute_1_time_midnight_sync():
    ...
    print(f'[{datetime.datetime.now().strftime("%H:%M:%S")} ] Starting Multi-Data Ingestion Pipeline on folder {target_season} for date {current_date}...')
    for league in LEAGUE_CATALOG:
        time.sleep(2.0)
        res = query_api_football('fixtures', {'league': league['id'], 'season': target_season, 'date': current_date})
    ...
    print(f'[{datetime.datetime.now().strftime("%H:%M:%S")} ] Master Multi-Data Sync Complete. Saved {len(fresh_map)} active game horizons inside season folder {target_season}.')
