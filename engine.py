...
MASTER_BOOKIE_CATALOG = [
    ...
    {"key": "soccer_usa_mls", "title": "USA MLS", "api_id": 253, "aliases": ["usa mls", "mls", "major league soccer", "usa major league soccer"]},
...
def clean_american_odds_to_int(odds_str):
    """
    Extracts an integer value from an American odds string parameter,
    safely handling positive (+), negative (-), and un-prefixed values.
    """
    try:
        clean_str = str(odds_str).strip()
        if not clean_str:
            return 100
        # Check if it starts with negative sign or contains it
        is_negative = clean_str.startswith("-")
        # Strip signs
        sanitized = clean_str.replace("+", "").replace("-", "")
        val = int(float(sanitized))
...
def is_any_valid_market_selection(odds_val):
    try:
        if odds_val is None:
            return False
        clean_str = str(odds_val).replace("+", "").replace("-", "").strip()
        _ = int(float(clean_str))
        return True
    except (ValueError, TypeError):
        return False

def get_live_pitch_telemetry(home_team, away_team, league_id=None):
    url = "https://api-sports.io/v3/football"
        ...
        if res.status_code == 200:
            for fx in res.json().get("response", []):
                ...
                if (home_team.lower()[:4] in h or h[:4] in home_team.lower()) or (away_team.lower()[:4] in a_team or a_team[:4] in away_team.lower()):
                    ...
                    try:
                        odds_res = requests.get(f"{url}/odds", headers=headers, params={"fixture": fx_id}, timeout=5)
...
def get_league_standings_and_audit(league_id, home_team, away_team):
    url = "https://api-sports.io/v3/football/standings"
    headers = {"x-rapidapi-key": API_FOOTBALL_KEY, "x-rapidapi-host": "v3.football.api-sports.io"}
            ...
            if res.status_code == 200:
                ...
                if records and len(records) > 0:
                    ...
                    if standings_lists and isinstance(standings_lists, list) and len(standings_lists) > 0:
                        ...
                        # Flat lookup list extraction
                        target_list = standings_lists[0] if isinstance(standings_lists[0], list) else standings_lists
                        
                        for team_entry in target_list:
                        ...
                        if h_found or a_found:
                            ...
                            # Fallback if games played in the current season are too low to offer standard trend data
...
def execute_global_pitch_sweeps():
    ...
    for sport_item in MASTER_BOOKIE_CATALOG:
        ...
        try:
            live_url = "https://api-sports.io/v3/football/fixtures"
                ...
                if fixtures_list:
                    ...
                    for fx in fixtures_list:
                        ...
                        if live_data.get("active"):
                            ...
                            # Clean and capture american odds values securely
                            if is_any_valid_market_selection(l_home_odds):
                                all_discovered_favorites.append({"team": h_name, "odds": clean_american_odds_to_int(l_home_odds), "match": f"{h_name} vs {a_name}", "league": league_title})
                            if is_any_valid_market_selection(l_away_odds):
                                all_discovered_favorites.append({"team": a_name, "odds": clean_american_odds_to_int(l_away_odds), "match": f"{h_name} vs {a_name}", "league": league_title})
                            
                            # SYSTEMS 1 & 7: Volatility and Late-Stage Pressure Window Alerts
                            if current_minute >= 45 and current_score == "0-0":
                                implied_p = convert_american_to_implied(clean_american_odds_to_int(l_home_odds))
                                interval_alert = (
    ...
    for sport_item in MASTER_BOOKIE_CATALOG:
        ...
        for fixture in match_data:
            ...
            if is_any_valid_market_selection(home_odds_val):
                all_discovered_favorites.append({"team": home, "odds": clean_american_odds_to_int(home_odds_val), "match": f"{home} vs {away}", "league": league_title})
            if is_any_valid_market_selection(away_odds_val):
                all_discovered_favorites.append({"team": away, "odds": clean_american_odds_to_int(away_odds_val), "match": f"{home} vs {away}", "league": league_title})

            implied_p = convert_american_to_implied(clean_american_odds_to_int(home_odds_val))
            ...
            if implied_p >= 0.55:
                try:
                    ...
                    fmt_h = f"+{home_odds_val}" if clean_american_odds_to_int(home_odds_val) > 0 else home_odds_val
                    fmt_d = f"+{draw_odds_val}" if clean_american_odds_to_int(draw_odds_val) > 0 else draw_odds_val
                    fmt_a = f"+{away_odds_val}" if clean_american_odds_to_int(away_odds_val) > 0 else away_odds_val
    ...
    if all_discovered_favorites:
        # Sort ascending by taking absolute value proximity into account or straight odds placement rankings
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if central_hour >= 23 or central_hour < 3:
            ...
            time.sleep(600)   # Active runtime sweep check frequency setting: 10 minutes
