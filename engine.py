...
# =====================================================================
# MASTER BOOKIE CATALOG: 51 LEAGUES STRUCTURAL VARIANCE SCOPE
# =====================================================================
...
TARGET_SPORTSBOOKS = ["bet365", "draftkings", "fanduel", "thescore", "circa", "hardrock", "williamhill_us", "pointsbetus", "sugarhouse", "bovada"]
...
def convert_american_to_implied(odds_val):
    ...
    except (ValueError, TypeError): return 0.50
...
def execute_global_pitch_sweeps():
    ...
    # 48-Hour Threshold for Current/Daily Board
    daily_threshold = current_time_utc + datetime.timedelta(hours=48)
    # 2 to 7 Day Window for System 6 Advanced Planning Board
    futures_start_window = current_time_utc + datetime.timedelta(days=2)
    futures_end_window = current_time_utc + datetime.timedelta(days=7)
    ...
    for sport_item in MASTER_BOOKIE_CATALOG:
        ...
        for fixture in match_data:
            ...
            if bookmakers_list and isinstance(bookmakers_list, list):
                for bm in bookmakers_list:
                    if isinstance(bm, dict):
                        ...
                        if bm_title in ["bet365", "draftkings", "fanduel", "bovada"] or bm_key in TARGET_SPORTSBOOKS:
                            target_bookmaker = bm
                            break
            ...
            # Formatted String Handling for Logs
            try:
                h_int = int(float(home_odds_val))
                h_fmt = f"+{h_int}" if h_int > 0 else str(h_int)
            except (ValueError, TypeError): h_fmt = "N/A"
            try:
                a_int = int(float(away_odds_val))
                a_fmt = f"+{a_int}" if a_int > 0 else str(a_int)
            except (ValueError, TypeError): a_fmt = "N/A"
            try:
                d_int = int(float(draw_odds_val))
                d_fmt = f"+{d_int}" if d_int > 0 else str(d_int)
            except (ValueError, TypeError): d_fmt = "N/A"

            # Check if match is running live right now
            is_live = commence_dt <= current_time_utc
            
            # Allocation Pipeline: Check Time Horizons
            match_payload_item = {"team": home, "odds": h_fmt, "match": f"{home} vs {away}", "league": league_title}
            
            if futures_start_window <= commence_dt <= futures_end_window:
                all_futures_favorites.append(match_payload_item)
            elif commence_dt <= daily_threshold:
                all_discovered_favorites.append(match_payload_item)

            # -----------------------------------------------------------------
            # HIGH-PRIORITY REAL-TIME ALERTS INTERCEPTOR
            # -----------------------------------------------------------------
            # System 2 Pre-Match Juice Interceptor Rules
            try:
                h_odds_int = int(float(home_odds_val))
                if -300 <= h_odds_int <= -175 and not is_live and commence_dt <= daily_threshold:
                    juice_alert = (
                        f"🏎️ **CORVETTE FUND BLUEPRINT — SYSTEM 2 JUICE OVERRIDE**\n\n"
                        f"**Match Context:** {home} vs {away} ({league_title})\n"
                        f"📈 **Pre-Match Line Alert:** Heavy Favorite ML Juice detected at ({h_fmt})\n"
            ...
            # System 7 Live Telemetry In-Play Logic Block
            if is_live:
            ...
            # Core Pure Value Structural Evaluation Trigger
            implied_p = convert_american_to_implied(home_odds_val)
            true_p = implied_p + 0.06
            edge_val = true_p - implied_p
            
            if edge_val >= 0.00 and commence_dt <= daily_threshold:
                try:
                    ...
                    full_alert = (
                        ...
                        f"* **Full-Time 1X2 Moneyline:** Home: {h_fmt} | Draw: {d_fmt} | Away: {a_fmt}\n"
    ...
    print(f"[+] Sweep Status: Checked 51 leagues. Found {leagues_with_data} leagues with active boards. Total matches evaluated: {total_matches_found}")

    # Pipeline Display 1: Aggregated Daily Favorites Board (Current/48 hrs)
    if all_discovered_favorites:
        board_msg = "🏎️ **CORVETTE FUND BLUEPRINT — TOP 20 DAILY MATCH BOARD**\n\n"
    ...
    # Pipeline Display 2: Aggregated System 6 Futures Planning Board (2 to 7 days away)
    if all_futures_favorites:
        futures_msg = "🏎️ **CORVETTE FUND BLUEPRINT — SYSTEM 6 ADVANCED FUTURES BOARD**\n\n"
        for index, item in enumerate(all_futures_favorites[:20], 1):
            futures_msg += f"{index}. **{item['team']}** ({item['odds']}) — *{item['match']}* [{item['league']}]\n"
