...
def send_discord_payload(content_str):
    if not DISCORD_WEBHOOK_URL:
        print("[-] Skipping Discord payload: DISCORD_WEBHOOK_URL environmental variable not set.")
        return
    time.sleep(0.5)
...
def query_api_football(endpoint, params):
    if not API_FOOTBALL_KEY:
        print("[-] Skipping API-Football call: API_FOOTBALL_KEY environmental variable not set.")
        return None
...
def fetch_odds_for_market(sport_key):
    if not THE_ODDS_API_KEY or THE_ODDS_API_KEY == 'YOUR_API_KEY':
        return None
    try:
