...
def send_discord_payload(content_str):
    try:
        ...
        clean_title = lines_list[0].replace("ðï¸", "").strip() if lines_list else "System Alert"
...
def execute_global_pitch_sweeps():
        ...
        for fixture in match_data:
            ...
            try:
                ...
                if -300 <= h_odds_int <= -175 and not is_live:
                    juice_alert = (
                        f"ðï¸ **CORVETTE FUND BLUEPRINT â SYSTEM 2 JUICE OVERRIDE**\n\n"
                        f"**Match Context:** {home} vs {away} ({league_title})\n"
                        f"ð **Pre-Match Line Alert:** Heavy Favorite ML Juice detected at ({home_odds_val})\n"
                        f"ð¯ **Operational Mandate:** Bypass direct standard line. Execute Time-Bracket strategy entry: **Goal Before 30:00** or **Favorite to Lead Before 30:00**."
                    )
            ...
            if is_live:
                ...
                if live_data.get("active"):
                    ...
                    if 0 <= current_minute <= 45 and current_score == "0-0":
                        interval_alert = (
                            f"ðï¸ **CORVETTE FUND BLUEPRINT â LIVE STRATEGY SIGNAL**\n\n"
            ...
            if not is_live and implied_p >= 0.55:
                try:
                    ...
                    full_alert = (
                        ...
                        f"**Match Context:** {home} vs {away} ({league_title}) â Pre-Match Audit\n"
    ...
    if all_discovered_favorites:
        ...
        board_msg = "ðï¸ **CORVETTE FUND BLUEPRINT â TOP 20 DAILY FAVORITES BOARD**\n\n"
        for index, item in enumerate(all_discovered_favorites[:20], 1):
            board_msg += f"{index}. **{item['team']}** ({item['odds']}) â *{item['match']}* [{item['league']}]\n"
...
if __name__ == "__main__":
    ...
    while True:
        ...
        test_payload = (
            f"ðï¸ **CORVETTE FUND ENGINE â STATUS VERIFIED**\n\n"
            f"ð¡ **Operational Status:** Active Loop Online\n"
            f"ð **Interval State:** Sweep Completed Cleanly\n"
            f"ð» **Server Core:** Render Node Live"
        )
                ...
                try:
                    with open(ledger_file, mode="r", encoding="utf-8") as f:
                        ...
                        for idx, row in enumerate(latest_records, 1):
                            ...
                            recent_rows_summary += f"ð¹ {clean_row}\n"
                except Exception as file_err:
                    print(f"[-] Ledger summary parsing exception: {file_err}")
            
            summary_banner = (
                f"ðï¸ **CORVETTE FUND ENGINE â 4-HOUR PERFORMANCE SUMMARY**\n\n"
                f"ð **Total Archived Records:** {total_logged_entries} Fired Signals\n"
                f"ð **Active System Health:** 100% Operational\n\n"
                f"ð **Most Recent Ledger Entries:**\n"
