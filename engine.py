...
def execute_global_pitch_sweeps():
        ...
        if live_res.status_code == 200:
            ...
            if fixtures_list:
                ...
                for fx in fixtures_list:
                    ...
                    # UNIVERSAL IN-PLAY CAPTURE RULE: Force landing execution if it belongs to your 51 tracking tiers
    ...
    if isinstance(match_data, list):
        for fixture in match_data:
            ...
            # SYSTEM OVERHAUL: UNIVERSAL SPORT INGESTION KEY - Fallback verification override layer
            fixture_sport_key = fixture.get("sport_key", "")
            matched_catalog_item = None
            
            # Match directly via sport_key mapping
            for sport_item in MASTER_BOOKIE_CATALOG:
                if sport_item["key"] == fixture_sport_key:
                    matched_catalog_item = sport_item
                    break
                    
            # UNIVERSAL RECOVERY: If specific key didn't match but it's a soccer sport type, generate structural catalog fallback proxy
            if not matched_catalog_item and "soccer" in fixture_sport_key:
                matched_catalog_item = {
                    "key": fixture_sport_key,
                    "title": fixture_sport_key.replace("soccer_", "").replace("_", " ").title(),
                    "api_id": 9999 # Safe fallback proxy index block
                }
