...
def execute_1_time_midnight_sync():
    current_date = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=5) # central standard approximation
    target_season = 2026
    fresh_map = {}
    
    now_str = (datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=5)).strftime("%H:%M:%S")
    print(f'[{now_str} ] Starting Paid-Tier Full League Schedule RAM Ingestion for folder {target_season}...')
        ...
        try:
            ...
            if res and res.get('response'):
                ...
                for item in fixtures:
                    ...
                    delta_days = (match_dt - datetime.datetime.now(datetime.timezone.utc)).days
                    ...
                    if 0 <= delta_days <= 2:
                        all_favorites.append(match_label)
                    elif 2 < delta_days <= 7:
                        all_futures.append(match_label)
    ...
    INTERNAL_STATE_MEMORY['top_20_favorites'] = all_favorites[:20]
    INTERNAL_STATE_MEMORY['system_6_futures'] = all_futures[:20]
    
    now_str2 = (datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=5)).strftime("%H:%M:%S")
    print(f'[{now_str2} ] Paid Ingestion Complete. Cached {len(fresh_map)} active game horizons into local memory arrays.')
    ...
    while True:
        try:
            ...
            for f_id, meta in list(active.items()):
                try:
                    match_time_str = meta['commence_time']
                    try:
                        match_dt = datetime.datetime.fromisoformat(match_time_str.replace('Z', '+00:00'))
                    except Exception:
                        continue
                        
                    # Target live telemetry checks for games happening today around the window
                    time_diff = (match_dt - datetime.datetime.now(datetime.timezone.utc)).total_seconds()
                    if -7200 <= time_diff <= 3600: # Active within a 2 hour back / 1 hour forward window
                        time.sleep(1.0)
                        ...
                        if check and check.get('response'):
                            ...
                            meta['status_short'] = current_status
