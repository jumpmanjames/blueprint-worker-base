...
def convert_decimal_to_american(odds_val):
    try:
        val = float(odds_val)
        if val >= 2.0:
            return f"+{int(round((val - 1) * 100))}"
        elif val > 1.0:
            return f"-{int(round(100 / (val - 1)))}"
    except Exception:
        pass
    return str(odds_val)
...
def is_any_valid_market_selection(odds_val):
    ...
    try:
        # Check if float (Decimal) or int string
        cleaned = str(odds_val).replace("+", "").replace("-", "")
        _ = float(cleaned)
        return True
    except ValueError:
        return False
...
def parse_multi_market_odds(bookmaker_data):
    ...
    # Handle list layer wrapping normalizer safely
    if isinstance(bookmaker_data, list) and len(bookmaker_data) > 0:
    ...
    for m in markets:
        ...
        if m.get("key") == "h2h":
            for outcome in m.get("outcomes", []):
                n = outcome.get("name")
                p = outcome.get("price")
                # Normalize decimal profiles if they appear on global streaming feeds
                if p and not str(p).startswith("+") and not str(p).startswith("-") and "." in str(p):
                ...
                elif str(n).lower() == "draw" or outcome.get("side") == "draw": odds_map["draw"] = p
        ...
        if m.get("key") == "totals":
            for outcome in m.get("outcomes", []):
                if outcome.get("name") == "Over":
                    ...
                    odds_map["home"] = p
                    try:
                        odds_map["away"] = f"+{int(str(p).replace('+','')) + 50}" if int(str(p).replace('+','')) > 0 else p
                    except Exception:
                        odds_map["away"] = p
                    odds_map["draw"] = "+200"
...
def get_league_standings_and_audit(league_id, home_team, away_team):
    ...
    return (
        ...
        f"2. **Positive Goal Differential:** {home_team} maintains tactical dominance with season performance ({h_gd_str} vs {a_gd_str}). **STATUS: PASS** 🟢\n"
    ...
    try:
        ...
        if live_res.status_code == 200:
            ...
            for fx in fixtures_list:
                ...
                # UNIVERSAL IN-PLAY CAPTURE RULE: Match using wide generic criteria safely
                if matched_catalog_item or fx.get("league", {}).get("type") == "League":
                    total_matches_found += 1
                    title_lbl = matched_catalog_item["title"] if matched_catalog_item else fx.get("league", {}).get("name", "Global League")
                    c_id = matched_catalog_item["api_id"] if matched_catalog_item else fx_league_id
                    
                    live_data = get_live_pitch_telemetry(h_name, a_name, c_id)
                    ...
                    clean_h_odds = int(str(l_home_odds).replace("+", "").replace("-", "")) if is_any_valid_market_selection(l_home_odds) else 100
                    clean_a_odds = int(str(l_away_odds).replace("+", "").replace("-", "")) if is_any_valid_market_selection(l_away_odds) else 100
                    
                    all_discovered_favorites.append({"team": h_name, "odds": clean_h_odds, "match": f"{h_name} vs {a_name}", "league": title_lbl, "kickoff": current_time_utc})
                    
                    if current_minute >= 45 and current_score == "0-0":
                        implied_p = convert_american_to_implied(l_home_odds)
                        interval_alert = f"""🏎️ **CORVETTE FUND BLUEPRINT — LIVE STRATEGY SIGNAL**
...
* **Why the data holds the edge:** Game clock verified at {live_data.get('clock')} mark sitting at balanced scoreline ({current_score}). Live attack velocity registers {live_data.get('dang_attacks_home')} Dangerous Attacks."""
    ...
    # Pipeline 2: Broad Pre-Match Horizon Ingestion via Universal Stream
    ...
    if isinstance(match_data, list):
        for fixture in match_data:
            # UNIVERSAL IN-PLAY TRACKING CAPTURE RULE INTERCEPT
            ...
            matched_catalog_item = None
            for sport_item in MASTER_BOOKIE_CATALOG:
                if sport_item["key"] == sport_key or any(a in home.lower() or a in away.lower() for a in sport_item["aliases"]):
                    matched_catalog_item = sport_item
                    break
            
            title_lbl = matched_catalog_item["title"] if matched_catalog_item else "Global Competition"
            api_id_lbl = matched_catalog_item["api_id"] if matched_catalog_item else 9999
            total_matches_found += 1
                
            target_bookmaker = None
            bookmakers_list = fixture.get("bookmakers", [])
            
            # Universal Bookmaker Fallback Rule Execution
            ...
            odds_payload = parse_multi_market_odds(target_bookmaker.get("markets", target_bookmaker))
            ...
            clean_h_odds = int(str(home_odds_val).replace("+", "").replace("-", "")) if is_any_valid_market_selection(home_odds_val) else 100
            clean_a_odds = int(str(away_odds_val).replace("+", "").replace("-", "")) if is_any_valid_market_selection(away_odds_val) else 100
            
            match_item = {
                "team": home, "odds": clean_h_odds, "match": f"{home} vs {away}", 
                "league": title_lbl, "kickoff": commence_dt,
            ...
            if time_delta_to_kickoff > datetime.timedelta(hours=48):
                ...
                all_discovered_favorites.append({"team": away, "odds": clean_a_odds, "match": f"{home} vs {away}", "league": title_lbl, "kickoff": commence_dt})

                implied_p = convert_american_to_implied(home_odds_val)
                
                # SYSTEM 2: Pre-Match Juice Entry Tracker Check
                juice_alert = f"""🏎️ **CORVETTE FUND BLUEPRINT — SYSTEM 2 JUICE OVERRIDE**

**Match Context:** {home} vs {away} ({title_lbl})
📈 **Pre-Match Line Alert:** Target Line Value detected at ({home_odds_val}) [Market: {odds_payload.get('market_used')}]
🎯 **Operational Mandate:** Bypass direct standard line. Execute Time-Bracket strategy entry: **Goal Before 30:00** or **Favorite to Lead Before 30:00**."""
                send_discord_payload(juice_alert)
                
                try:
                    system_5_details = get_league_standings_and_audit(api_id_lbl, home, away)
                    full_alert = f"""🏎️ **CORVETTE FUND BLUEPRINT — MARKET ANALYSIS SYSTEM**

**Match Context:** {home} vs {away} ({title_lbl}) — Upcoming: {commence_dt.strftime('%b %d at %I:%M %p')} Central
...
🏎️ **CORVETTE FUND BLUEPRINT — MARKET ANALYSIS SYSTEM**
...
if __name__ == "__main__":
    ...
    while True:
        ...
        test_payload = f"""🏎️ **CORVETTE FUND ENGINE — STATUS VERIFIED**
