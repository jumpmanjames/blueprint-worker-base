...
def evaluate_live_inplay_telemetry(fixture_id, match_details, h2h_odds_matrix):
    try:
        ...
        stats_data = {"Shots on Goal": "N/A", "Attacks": "N/A", "Dangerous Attacks": "N/A", "Ball Possession": "N/A", "Corner Kicks": "N/A", "Red Cards": "N/A", "Yellow Cards": "N/A"}
        
        if res and res.get('response'):
            for team_stats in res['response']:
                for s in team_stats.get('statistics', []):
                    type_str = s.get('type')
                    if type_str in stats_data:
                        stats_data[type_str] = str(s.get('value') or 0)
        ...
        report = f'🏎️ **CORVETTE FUND BLUEPRINT — SYSTEM SELECTION TRIGGERED**\n\n' \
                 ...
                 f'**Live System 7 Telemetry Metrics:**\n' \
                 f'• Shots on Goal: {stats_data["Shots on Goal"]}\n' \
                 f'• Attacks / Dangerous Attacks: {stats_data["Attacks"]} / {stats_data["Dangerous Attacks"]}\n' \
                 f'• Possession: {stats_data["Ball Possession"]}\n' \
                 f'• Corners / Discipline: {stats_data["Corner Kicks"]} | Reds: {stats_data["Red Cards"]} Yellows: {stats_data["Yellow Cards"]}'
                 
        send_discord_payload(report)
        log_to_ledger(fixture_id, match_details['league_name'], teams, odds_data, 'SYSTEM_BLUEPRINT_SELECTION')
    except Exception as e:
        print(f'[-] Telemetry extraction error on {fixture_id}: {e}')
...
def execute_1_time_midnight_sync():
    ...
    print(f'[{now_ct.strftime("%H:%M:%S")} ] Starting Paid-Tier Full League Schedule RAM Ingestion for folder {target_season}...')
        ...
        try:
            ...
            if res and res.get('response'):
                ...
                for item in fixtures:
                    ...
                    try:
                        match_dt = datetime.datetime.strptime(match_time_str[:19], '%Y-%m-%dT%H:%M:%S')
                    except ValueError:
                        continue
                    ...
                    delta_days = (match_dt - now_ct).days + (match_dt - now_ct).seconds / 86400.0
                    match_label = f'🔹 {home} vs {away} ({league["name"]}) - {match_time_str}'
                    
                    if 0 <= delta_days <= 2:
                        fav_candidates.append((delta_days, match_label))
                    elif 2 < delta_days <= 7:
                        fut_candidates.append((delta_days, match_label))
    ...
    fav_candidates.sort(key=lambda x: x[0])
    fut_candidates.sort(key=lambda x: x[0])
    
    INTERNAL_STATE_MEMORY['top_20_favorites'] = [x[1] for x in fav_candidates[:20]]
    INTERNAL_STATE_MEMORY['top_20_futures'] = [x[1] for x in fut_candidates[:20]]
    
    print(f'[{now_ct.strftime("%H:%M:%S")} ] Paid Ingestion Complete. Cached {len(fresh_map)} active game horizons into local memory arrays.')
    ...
    if INTERNAL_STATE_MEMORY['top_20_futures']:
        send_discord_payload('🚀 **TOP 20 FUTURE MATCHUPS BOARD (2-7 DAYS OUT)**\n' + '\n'.join(INTERNAL_STATE_MEMORY['top_20_futures']))
...
def initialize_automation_pipeline():
    ...
    while True:
        try:
            now_ct = datetime.datetime.utcnow() - datetime.timedelta(hours=5)
            current_date_str = now_ct.strftime('%Y-%m-%d')
            ...
            # Age-out mechanism: clear finished games from memory loop
            finished_keys = []
            
            for f_id, meta in list(active.items()):
                if meta['commence_time'][:10] != current_date_str:
                    continue
                try:
                    time.sleep(1.0)
                    ...
                    if check and check.get('response'):
                        ...
                        elif current_status in ['1H', 'HT', '2H', 'ET', 'P', 'LIVE']:
