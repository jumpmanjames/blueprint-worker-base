...
def execute_1_time_midnight_sync():
    ...
    print(f'[{datetime.datetime.now().strftime("%H:%M:%S")} ] Starting Paid-Tier Full League Schedule RAM Ingestion for folder {target_season}...')
    for league in LEAGUE_CATALOG:
        try:
            ...
            if res and res.get('response'):
                ...
                # Filter down to today's matches out of the full season schedule locally in RAM
                for item in fixtures:
                    match_date_str = item['fixture']['date'][:10]  # Extracts YYYY-MM-DD
    ...
    print(f'[{datetime.datetime.now().strftime("%H:%M:%S")} ] Paid Ingestion Complete. Cached {len(fresh_map)} active game horizons into local memory arrays.')
