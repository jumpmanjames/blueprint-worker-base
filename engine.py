        ...
        try:
            ...
            if res and res.get('response'):
                ...
                for item in fixtures:
                    ...
                    match_date_str = item['fixture']['date']
                    ...
                    fresh_map[f_id] = match_profile
                    
                    # Sort into Favorites vs Futures pipelines based on timeline definitions
                    if 0 <= delta_days <= 2:
                        INTERNAL_STATE_MEMORY['top_20_favorites'].append(f'🔹 {home} vs {away} ({league["name"]}) - {match_date_str}')
                    elif 2 < delta_days <= 7:
                        INTERNAL_STATE_MEMORY['system_6_futures'].append(f'🔮 {home} vs {away} ({league["name"]}) - {match_date_str}')
...
def initialize_automation_pipeline():
    ...
    while True:
        try:
            ...
            print(f'[{now_utc.strftime("%H:%M:%S")} ] Heartbeat Loop Active: Scanning {active_count} soccer game arrays across priority bookmakers (Bet365/DraftKings)...')
            ...
            for f_id, meta in active:
                try:
                    ...
                    if check and check.get('response'):
                        ...
                        # Data Age-Out: Drop from local memory array if completed to prevent session bloat
