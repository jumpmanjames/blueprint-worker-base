    ...
    try:
        ...
        if res.status_code == 200:
            for fx in res.json().get("response", []):
                ...
                # Broadened match signature logic to catch multi-variant strings cleanly
...
def is_any_valid_market_selection(odds_val):
    if odds_val is None:
        return False
    try:
        clean_odds = str(odds_val).replace("+", "").replace("-", "").strip()
        if clean_odds.isdigit():
            return True
        return False
    except ValueError:
        return False
                    ...
                    for fx in fixtures_list:
                        ...
                        if live_data.get("active"):
                            ...
                            if is_any_valid_market_selection(l_home_odds):
                                raw_odds = int(str(l_home_odds).replace("+", "").strip())
                            ...
                            if is_any_valid_market_selection(l_away_odds):
                                raw_odds = int(str(l_away_odds).replace("+", "").strip())
    ...
    for sport_item in MASTER_BOOKIE_CATALOG:
        ...
        for fixture in match_data:
            ...
            if is_any_valid_market_selection(home_odds_val):
                all_discovered_favorites.append({"team": home, "odds": int(str(home_odds_val).replace("+","")), "match": f"{home} vs {away}", "league": league_title})
            if is_any_valid_market_selection(away_odds_val):
                all_discovered_favorites.append({"team": away, "odds": int(str(away_odds_val).replace("+","")), "match": f"{home} vs {away}", "league": league_title})
