...
# Explicitly pairs your 51 Master Leagues between both API ecosystems to eliminate 0-cache drops
MASTER_LEAGUE_MAP = {
...
def send_heartbeat():
    ...
    payload = {
        "embeds": [{
            "title": "🏎️ CORVETTE FUND PERMANENT DIRECTORY ONLINE",
            "color": 65280,
            "description": f"✅ **Permanent RAM Directory Engine Initialized.**\nLoaded {len(MASTER_LEAGUE_MAP)} unshakeable league translations into local memory storage."
        }]
    }
        ...
        try:
            ...
            if pm_res.status_code == 200:
                for fix in pm_res.json():
                    ...
                    # Unrestricted display loop to force match visibility onto Render logs instantly
        ...
        try:
            ...
            if live_res.status_code == 200:
                for l_fix in live_res.json():
                    ...
                    # Direct correlation mapping using the permanent lookup directory parameters
