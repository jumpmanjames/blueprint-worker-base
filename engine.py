...
def execute_1_time_midnight_sync():
    ...
    print(f'[{datetime.datetime.now().strftime("%H:%M:%S")} ] Starting League-Iterated Month RAM Ingestion on folder {target_season}...')
    for league in LEAGUE_CATALOG:
        ...
        if res and res.get('response'):
            ...
            for item in fixtures:
                ...
                f_date_str = item['fixture']['date'][:10]
                try:
                    f_date = datetime.datetime.strptime(f_date_str, '%Y-%m-%d')
                    if abs((f_date - current_date).days) > 15:
                        continue
                except Exception:
                    pass
    ...
    print(f'[{datetime.datetime.now().strftime("%H:%M:%S")} ] Master RAM Multi-Data Ingestion Complete. Saved {len(fresh_map)} multi-week game horizons inside RAM arrays.')
